...
- `Rental.penalty_value` holds the **replacement price of the pieces** ("valor de reposição"), not a late fee — confirmed with the shop on 2026-08-02 and by the legacy migration, where `locado.multa` is constant per rental and worth 1.2x–3x the rental. It prints in clause 3 of the contract. The field name is historical; do not rename it (the legacy importer and ~30 test payloads depend on it).
  - It is **not** the late-return fee. That was settled with the shop on 2026-08-02: `movements.services.compute_penalty` charges `Company.late_return_daily_rate` (10%) of `rental.final_value` per day, for at most `Company.late_return_max_days` (7) days. Past that cap the garment counts as not returned and contract clause 6 charges `loss_penalty_rate` instead — the cap is what keeps the two clauses from overlapping. The rate is printed in clause 4, so it is contractually stated; changing either field changes the contract text too.
