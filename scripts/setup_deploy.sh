...
# =============================================================================
#  Noivas & Cia — deploy do zero numa VPS Ubuntu limpa, seguindo a mesma
#  arquitetura documentada em docs/deploy/PRD-deploy.md e docs/deploy/guia-vps.md
#  (portado do template usado no projeto SCSI).
#
...
#        basic auth do Traefik, secret da Cloudflare, deploy da stack.
#
...
# URL pública do próprio script (fallback quando rodado via 'bash -c "$(curl ...)"',
# sem arquivo em disco). Ajuste SETUP_DEPLOY_URL se hospedar o script em outro lugar.
SCRIPT_URL="${SETUP_DEPLOY_URL:-}"
...
phase_system() {
  ...
  PROJECT_NAME="$(ask "Nome do projeto (igual ao repositório)" "noivaecia")"
  while [[ -z "$PROJECT_NAME" || "$PROJECT_NAME" =~ [[:space:]] ]]; do
    warn "Informe um nome sem espaços (ex: noivaecia)."
    PROJECT_NAME="$(ask "Nome do projeto (igual ao repositório)" "noivaecia")"
  done
  ...
  # ── fail2ban ──
  # No Ubuntu 24.04 a jail padrão do SSH chama-se [sshd] (não [ssh]) e o
  # backend correto é systemd (o rsyslog tradicional em /var/log/auth.log
  # não é a fonte de log ativa por padrão). Uma jail [ssh] roda como
  # no-op silencioso — o fail2ban aceita a config mas nunca bane nada.
...
EOF
  ...
  ok "fail2ban ativo"
  if fail2ban-client get sshd maxretry >/dev/null 2>&1; then
    ok "Jail 'sshd' confirmada ativa (fail2ban-client get sshd maxretry = $(fail2ban-client get sshd maxretry 2>/dev/null))"
  else
    warn "Não consegui confirmar a jail 'sshd' via fail2ban-client — verifique manualmente."
  fi
...
EOF
  ...
  # NOPASSWD facilita o fluxo do script; endurecer depois do go-live é
  # recomendado (ver docs/deploy/PRD-deploy.md, tabela de riscos).
  ...
  if [[ -n "$self" && -f "$self" ]]; then
    ...
    if ! curl -fsSL "$SCRIPT_URL" -o "$dest" 2>>"$LOG_FILE"; then
      ...
      action_box \
        ...
        "  su - ${DEPLOY_USER}" \
        "  bash setup_deploy.sh   (copie este arquivo para lá antes)"
      exit 0
    fi
  else
    warn "Sem arquivo em disco e sem SETUP_DEPLOY_URL definido."
    action_box \
      "A FASE 1 (servidor) terminou com sucesso. Para a FASE 2 (deploy):" \
      "  su - ${DEPLOY_USER}" \
      "  bash setup_deploy.sh   (copie este arquivo para lá antes)"
...
phase_deploy() {
  ...
  [[ -n "$PROJECT_NAME" ]] || PROJECT_NAME="$(ask "Nome do projeto (igual ao repositório)" "noivaecia")"
  while [[ -z "$PROJECT_NAME" || "$PROJECT_NAME" =~ [[:space:]] ]]; do
    warn "Informe um nome sem espaços."; PROJECT_NAME="$(ask "Nome do projeto (igual ao repositório)" "noivaecia")"
  done
...
SSHCFG
  ...
  if [[ -d "${PROJECT_DIR}/.git" ]]; then
    ...
    REPO_SSH="$(ask "URL do repositório em SSH" "git@github.com:elvertoni/noivaecia.git")"
    if ! retry git clone "$REPO_SSH" "$PROJECT_DIR" >>"$LOG_FILE" 2>&1; then
      warn "Clone via SSH falhou. Vamos tentar via HTTPS."
      REPO_HTTPS="$(ask "URL do repositório em HTTPS" "https://github.com/elvertoni/noivaecia.git")"
  ...
  [[ -n "$IMAGE" ]] || IMAGE="ghcr.io/elvertoni/noivaecia"
  ...
  if [[ -s .env ]] && ! confirm_SN ".env já existe e tem conteúdo. Quer substituir?"; then
    ...
    action_box \
      ...
      "(é o mesmo .env que você usa, com DJANGO_DEBUG=False e os valores de produção)" \
      "" \
      "No editor nano:" \
  ...
  if [[ -z "$DOMAIN" ]]; then
    ...
    if [[ -n "$resolved" ]]; then
      info "Agora ${DOMAIN} resolve para: ${resolved}"
    else
...
print_final_instructions() {
  ...
  echo ""
  echo "  ${BOLD}Migração de dados (import do legado BRcom):${RESET}"
  echo "    ${GREY}Ver docs/deploy/runbook-cutover-legado.md antes de rodar import_legacy_access.${RESET}"
  echo ""
...
if [[ "${1:-}" != "--as-deploy" ]]; then
  banner "🚀  SETUP DE VPS + DEPLOY — Noivas & Cia"
  echo "  Este script faz o deploy COMPLETO do sistema, do zero."
  ...
  echo "    • O conteúdo do ${BOLD}.env de produção${RESET} do projeto (você vai COLAR ele)"
  ...
  echo ""
  echo "  ${GREY}Dica: deixe DOMAIN preenchido no .env — ele controla o domínio do${RESET}"
