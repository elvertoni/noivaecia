...
# ── Noivas & Cia — Deploy completo (Docker Swarm, executado NA VPS) ──
# Uso:
...
#   - arquivo .env preenchido (DJANGO_DEBUG=False; DJANGO_ALLOWED_HOSTS com
#     localhost p/ o healthcheck do container)
...
echo "=== Noivas & Cia — Deploy ($STACK_NAME) ==="
echo ""
...
info ".env carregado"
URL="https://${DOMAIN:-<DOMAIN ausente no .env>}"
...
[ "${DJANGO_DEBUG:-}" = "False" ] || warn "DJANGO_DEBUG não está 'False' (valor atual: '${DJANGO_DEBUG:-<vazio>}') — produção deve usar DJANGO_DEBUG=False."
case ",${DJANGO_ALLOWED_HOSTS:-}," in
    *,localhost,*) : ;;
    *) warn "DJANGO_ALLOWED_HOSTS não contém 'localhost' — o healthcheck do app vai falhar (DisallowedHost)." ;;
esac
...
if [ "$SKIP_BUILD" -eq 0 ]; then
    ...
    if ! docker push "$IMAGE:$VERSION"; then
        ...
        echo "$GITHUB_TOKEN" | docker login ghcr.io -u elvertoni --password-stdin
