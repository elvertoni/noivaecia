...
- [x] Virada na VPS (§8, adaptada para PostgreSQL) — 20/07/2026 ~00:00 (BRT). Produção usa Postgres 16 (`work/pg`, banco `noivas_cia`), não mais SQLite em volume. Executado: backup `pg_dump -Fc` prévio (`/var/lib/postgresql/data/backups/noivas_cia-pre-migra-20260720.dump`, 5,5MB, dentro do volume do pg); transferência direta SQLite local → Postgres VPS via `tools/db_transfer/sqlite_to_pg.py` (493.057 linhas, 30 tabelas incl. `legacy_*`, transação única, sequences resetadas, ~77s) através de túnel TCP temporário (forwarder Python no container do app, porta 55432→15432→pg:5432, derrubado após a cópia); verificação `tools/db_transfer/verify_pg_migration.py` — **55/55 checks OK** (contagens, checksums financeiros exatos: receivable 9.269.675,00 / paid 4.564.328,00 / payments 6.340,00 / movements 5.898.446,00 / rentals 9.478.549,00; sequences; invariantes; spot-checks).
- [x] Smoke test em produção (§8) — HTTPS 200 em `noivasecia.tonicoimbra.com`, ORM no container lendo 18.831 clientes / 35.813 locações, user `elvertoni@gmail.com` (superuser) preservado. Pendências pós-virada: (a) remover na UI do EasyPanel o port mapping 55432→15432 do serviço `noivaecia` (inerte — nada escuta — mas melhor limpar); (b) usuário de acesso da Ana precisa ser recriado (o pg antigo tinha 2 users; a carga substituiu pelos users locais = só elvertoni); (c) registrar horário de corte com a cliente.
